...
`locater` is an R package for testing tree structures (represented via a set of discrete clades and/or a relatedness matrix) for association with traits of interest. 

This software and the underlying LOCATER methodology is described in a peer reviewed paper accepted and to appear in Genetics:
...
An early pre-print of this paper is available here:
...
We recently applied `locater` to a richly phenotyped whole genome sequencing described in our pre-print here https://www.medrxiv.org/content/10.1101/2024.11.04.24316696. 

Please see the package website, https://ryanchrist.github.io/locater/ , for documentation and an Article presenting toy example for getting started: https://ryanchrist.github.io/locater/articles/simple_gwas_example.html . This article also demonstrates how the locater package API may be used to test inferred clades and relatedness matrices generated by other ancestry inference methods. The scripts, code, and data used to generate the simulation results presented in our paper can be found here: https://github.com/ryanchrist/locater_paper_scripts .
...
For those familiar with Docker, the brief Dockerfile below provides a minimal installation of the latest version of `locater`
```
FROM rocker/r-ver:latest
RUN apt-get update && apt-get install -y --no-install-recommends git zlib1g-dev
RUN R -e "install.packages('remotes')"
RUN R -e "remotes::install_bioc('rhdf5'); stopifnot(require(rhdf5))"
RUN mkdir ~/.R && echo 'CFLAGS= -march=native -mtune=native -O3' > ~/.R/Makevars
RUN R -e "remotes::install_github('louisaslett/kalis'); stopifnot(require(kalis))"
RUN R -e "remotes::install_github('ryanchrist/locater'); stopifnot(require(locater))"
```

For those unfamiliar with Docker or aiming to install locater on their own system in a more traditional way, please follow the steps below (equivalent to the Dockerfile instructions).
