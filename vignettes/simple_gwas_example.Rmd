...
y[,1] <- y[,1] + colSums((g[,seq(1,N(),2)] + g[,seq(2,N(),2)])*c(150,10,30)#c(150,8,20)
                         /rowSums(g))
...
test.opts <- data.frame(
  "smt.noise" = c(TRUE,TRUE,FALSE,FALSE,"raw"), # Clade-free testing options (eg: SMT, might be more complex)
  "thresh" = 0.2, 
  "old.sprigs" = c(FALSE),# Clade calling options
  "max1var" = c(TRUE,FALSE,TRUE,FALSE,TRUE),
  "max.eigs" = 250L
  )
