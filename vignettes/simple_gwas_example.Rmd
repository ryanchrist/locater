...
test.opts <- data.frame(
  ...
  "mixed.sprigs" = TRUE # Clade testing options
)
...
approx.res <- TestLoci(y, pars, target.loci = target.loci, sw.approx = TRUE, test.opts = test.opts)
...
exact.res <- TestLoci(y, pars, target.loci = new.target.loci, sw.approx = FALSE, test.opts = test.opts)
