...
for(i in 1:n.reps){
  ...
  # For rare var with 3 copies that's really interesting because it has no samples exclusively 
  # attributed to it -- maybe this is an example where having two layers of Renyi distillation would
  # help ... 
  #causal.idx <- 2845
  #target.idx <- 2757
  
  # here's another tripleton example:
  causal.idx <- 2842
  target.idx <- 2880
...
# Let's look at outlier signal we get if we ignore dependence 
...
# find the rows and columns (the sprigs and their samples) that tag the causal var
which(g > 0)
which(D[which(D %*% g > 0),]>0,arr.ind = T)
